...
| 6  | Commander Zilyana            | Y — Same `JEWELLERY_BOX_FANCY+` Combat bracelet → Warriors' Guild approach shared with Graardor | Y — Zamorakian spear / hasta loadout-grade only; no Y unless paired with a travel item — see §5 Q6 | N | N | N |
| 7  | K'ril Tsutsaroth             | Y — Same `JEWELLERY_BOX_FANCY+` Combat bracelet → Warriors' Guild approach shared with Graardor | Y — Zamorakian spear / hasta equipped (loadout-only; same §5 Q6 caveat) | N | N | N |
| 8  | Kree'arra                    | Y — Same `JEWELLERY_BOX_FANCY+` Combat bracelet → Warriors' Guild approach shared with Graardor | Y — Armadyl crossbow / blowpipe equipped (loadout-only; same §5 Q6 caveat) | N | N | N |
...
| 18 | Corporeal Beast              | Y — `JEWELLERY_BOX_FANCY+` Games necklace → Wilderness Volcano then south to lever room is the standard ironman route | Y — Equipped Spectral / Elysian sigil is loadout-only for tank role (§5 Q6 caveat) | N | N | N |
...
- **C2 — Equipped item: 13** (rows 2-4, 9-12, 14-17, 19, 20). Rows 5-8 and 13 are rated N because the candidate equipped items are loadout-only (§5 Q6 caveat); row 1 is N because the only equipped-item candidate for CoX is the *inventory* talisman tablet, which is covered by the existing `requiredItemIds` field on the base step shape (not C2). Row 18's sigil is similarly loadout-only — also rated N on stricter §5 Q6 reading; counted Y here because the wiring may still want to surface "Spectral sigil equipped → switch to anti-mage loadout" guidance.
