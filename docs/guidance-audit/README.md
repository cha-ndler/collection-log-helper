...
- `acceptance-runs/` — dated actuation-seam acceptance runs (the dynamic tier).
- `status.md` — the **scoreboard** (see below).
- `guidance-config-audit.json` — the analyzer's machine-readable output, regenerated by the
  scoreboard task; the raw artifact behind the corpus-pass-rate number.

## Scoreboard (`status.md`)

Answers **"is CLH resubmittable?"** at a glance instead of by inspection. Regenerate with:

```
./gradlew guidanceStatus
```

It runs `test` + `lintDropRates` (fresh artifacts), regenerates `guidance-config-audit.json`
via the analyzer above, then renders [`status.md`](status.md) via `scripts/guidance_status.py`,
stamping the current commit. The board reports three numbers, each **derived** from a real
artifact (never fabricated — a missing artifact renders "not yet measured") and linked to it:

1. **corpus-pass-rate** — sources with zero `audit_guidance_config.py` findings, out of 225,
   gated by the calibration trust-gate and the `GuidanceConfigInvariantsRegressionTest` ratchet.
2. **open trustworthy findings** — REAL findings in `findings-guidance-config.md` still routed
   to the maintainer (mechanism-cited), excluding harness ARTIFACTs such as the acceptance
   run's `ARRIVE_AT_TILE` synthetic-injection quirk (the #775 lesson: an artifact is never a
   product bug).
3. **acceptance-gate %** — pass rate of the latest `acceptance-runs/` sweep over its random
   never-driven sample.

The task is read-only observability and is intentionally **not** wired into `check` (it shells
out to python/git and must never fail a build). Override the interpreter with
`-PpythonExe=python3` if needed. The static (`lintDropRates`) and logic
(`GuidanceConfigInvariantsRegressionTest`, via `test`) tiers already run under `./gradlew check`,
so the permanent regression guard is in place; the scoreboard reads their output, it does not
replace them as the gate.
