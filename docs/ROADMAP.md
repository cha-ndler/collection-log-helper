...
> **Status**: Living document. Update the status block at the end of each milestone or agent session. Last reviewed: 2026-05-25.
...
8. **No external tool attribution in commits**. No "co-authored-by" or "generated by" lines, full stop.
...
> **Next orchestration session (2026-05-26 handoff).** **D4 and D5 are both closed.** Every source sits at the full or long-tail guidance bar (D4), and player-state conditional alternatives (POH teleports, diaries, equipped/inventory teleport items, skill-cape perks) are layered across the source set (D5-A/B/C/E/F/H merged via #690-#697; D5-D and D5-G dropped as null results; `questMilestones` field exists but has no data users yet). No D-tier coverage work remains queued.
> The next milestone is a product decision, not more data wiring. Candidates:
> 1. **Tier A / v1.0.0-hub readiness (A6)** -- still HOLD. The plugin-hub resubmission checklist (`docs/plugin-hub-review.md`, §2) is the gating work; the quiet-week countdown restarts on the latest hub-relevant merge. Pick this up only on explicit direction.
> 2. **Tier B.5 UI parity** or **Tier E cross-source meta** -- both now unblocked by the completed B/C/D foundation.
> Bug follow-ups still on file from validation: deferred **Barracuda Trials** (B4.3.5, re-audit post-Sailing), and in-client validation of the Sailing coords flagged in #671/#685.
...
- **Privacy in commits is non-negotiable** — author name `cha-ndler`, email is GitHub noreply, no external tool attribution, no real first names anywhere in source.
...
- [?] A6 — Tag v1.0.0-hub, wait a week, resubmit         status: blocked       owner: cha-ndler  updated: 2026-05-22  pr: #355, refresh pending  note: **HOLD — do not tag or resubmit.** All 2026-05-16 validation issues now closed: #483, #484, #485, #486 (closed by #608 + #616 — state-source correctness fixed at the impl-class level via `@Subscribe` wiring; see late-2026-05-22 entry below), #487 (re-confirmed closed by #583 quest-count fix), #488 (F1/F2 sync UX). 2026-05-19 hot-fix: PR #543 added `@ImplementedBy` to four C-tier player-state interfaces (`DiaryTierState`, `EquippedItemState`, `PohTeleportInventory`, `SkillCapePerkState`) after live `./gradlew run` surfaced a `PluginInstantiationException` regression latent since PRs #459/#461 — DI was unbound for those four interfaces and any Plugin-Hub build would have failed to load on a fresh client. Regression guard `CollectionLogHelperPluginInjectorTest` added to catch this class of bug at CI time. 2026-05-21 Pass 2 closure: audit umbrella #495 closed after all five carry-over regressions (#574, #575, #576, #486-reopen, #487-reopen) and two follow-up enhancements (#573, #577) merged across PRs #580-#587. Post-#503 god-class residuals re-measured below 800 floor: Plugin 488, Panel 605, Coordinator 557, Sequencer 623, EfficiencyCalculator 777 (was 851; the only file above the floor at the previous measurement is now under). Quiet-week countdown restarts from the latest merge in this content set. User decision 2026-05-16: "i don't want to deploy this to the plugin hub until things are fully functional." 2026-05-22 Pass 3 outcome (PR #601 against master `966a761e`): of 7 post-Pass-2 fixes re-tested + 4 carry-overs, **3 verdicts verified working**, **3 regressions filed**, **3 new blockers opened**. New tier-A blocker **#598** — guidance auto-advances to next clog item on last-step completion, abandoning the current source mid-grind for any rare/keyed drop (Shades, Larran's, most boss drops); must gate on actual slot unlock or explicit stop. Tier-A blocker **#599** closed by PR #603 (2026-05-22, Recommended chip strip hoisted to source-level Requirements area). Tier-B blocker **#600** closed by PR #602 (2026-05-22, F1/F2 tooltip trim). #495 remains open; v1.0.0-hub tag still HOLD.

  2026-05-23 Tier-A blocker closure: **both remaining hub blockers from the Pass 3 cycle are now closed.** **#598** (auto-advance abandons current source on rare/keyed drops) closed by PR **#605** (2026-05-22, gates auto-advance on actual target-slot unlock rather than last-step completion). **#575** (re-opened wrap-boundary character drop in side-panel step text) closed by PR **#606** (2026-05-22, swapped step-text `JLabel` for `JTextArea` so wrapping no longer drops glyphs on the wrap boundary). With those merges, the open-issue counts are now: **`tier-A` = 0**, **`hub-blocker` = 0**, **`priority: P0` = 0**, **`priority: P1` = 0**. The quiet-week countdown restarts from the latest hub-relevant merge in this content set (PR #628 at 2026-05-23T23:10Z, the C6 B1-batch closure doc; see Tier C entry below). User decision 2026-05-16 still standing: "i don't want to deploy this to the plugin hub until things are fully functional." v1.0.0-hub tag remains HOLD until the quiet-week elapses with no regressions surfaced.

  2026-05-22 (late, continued) Tier-C unblock: **C6 batch plan opened.** Scoping doc landed in **#621** with §3.2 per-detector mapping and §4 batch plan (B0 schema → B1 C2 equipped-items ×13 → B2 C1 POH-teleports ×8 → B3 C3 diaries ×6; C4 dropped, C5 absorbed into existing `quests` field). B0 schema PR **#623** merged the same session: two new `@Nullable` list fields on `SourceRequirements` (`pohTeleports`, `equippedItemIds`) wired into the conditional-alternative evaluator (`RequirementsChecker.checkRequirementsDetail`) as AND-clauses. `RequirementsChecker`'s constructor now injects `PohTeleportInventory` + `EquippedItemState` (both already `@ImplementedBy` so Guice resolves transparently). 28 new tests cover Gson absent/null/empty/populated for each new field, legacy-JSON backwards compatibility, AND-semantics, and first-match-wins alternative-selection. No data changes (`drop_rates.json` unchanged); no diaries-gating change (out of B0 scope per §3.2). B1 (C2 equipped-items wiring on 13 sources) is the next milestone, with a charge-aware-fallback authoring requirement called out at §5 Q4: B1 alternatives gating on a chargeable equip (Drakan's medallion ×4, ring of shadows ×4, Quetzal whistle ×1) must each pair with a non-equipped fallback alternative because `EquippedItemState.hasEquipped(int)` returns true even at 0 charges.

  2026-05-22 (late) Tier-A blocker closure: **#486 is now fully closed at the root-cause level.** The earlier #583 closure rendered C1-C5 rows in the C7 debug overlay but did not refresh detector state when the underlying game events fired, so the values shown could go stale. PR **#608** landed a diagnostic-overlay-only workaround (`PlayerCapabilityDebugOverlay` pulls a fresh snapshot from each Tier-C state source on every paint). Issue **#611** was filed and closed in this same session as the proper fix: PR **#616** wires `@Subscribe` handlers directly on the C1-C5 state-source impl classes (registered in `CollectionLogHelperPlugin.startUp()`, unregistered in `PluginShutdownRoutine.tearDown()`), so any consumer — overlay, sequencer, conditional alternative, helper — now sees live state from event-fire onwards. #608's in-paint refresh stays as belt-and-suspenders. **#487** also re-confirmed closed by #583's quest-count fix and not regressed in this session. Quiet-week countdown restarts from the latest merge in this content set.

  2026-06-04 hub auto-validation sweep (HEAD `94756851`): cleared the last open code/data FAIL on the v1 hub checklist. **T3.1** (Nightmare / Phosani's Nightmare step 1 never auto-advancing on Drakan's-medallion / shortcut entry to Slepe) fixed by PR **#731** — step 1 on both sources converted from `ARRIVE_AT_TILE` r=20 to `ARRIVE_AT_ZONE [3700,3290,3770,3360,0]` over Slepe town; data-only, no code change (the `ARRIVE_AT_ZONE` predicate already shipped). Verified out-of-game: `./gradlew build`+`test` green (1752 tests), `validate_drop_rates` / `data_ascii_lint` / `commit_message_lint` clean. Also confirmed **T1.5** (#484) data already correct — every Asgarnian Ice Dungeon source reads fairy ring `AIQ` (Mudskipper Point), `AKQ` reserved for Piscatoris/Kraken. With T3.1 closed, the v1 hub checklist has **0 open code/data FAILs**; all remaining gating rows are in-game-only (combat, hard clue, depleting loop, target-in-render). In-game confirm recipes captured in `docs/post-merge-ux-confirmation.md`. PR #731 also lists a teleport/shortcut-entry audit (Ring-of-Shadows DT2 bosses, ToB Drakan's-medallion entry) for follow-up data passes. Quiet-week countdown restarts from PR #731 (2026-06-04); earliest hub resubmission = 7 days after the final merge in this content set.
...
- [ ] B1 — Composable completion conditions             status: partial       owner: —          updated: 2026-05-22  note: Infrastructure landed piecemeal — `CompletionCondition` enum carries 11 condition types (item, inventory, tile, zone, NPC, plane, death, chat, VARBIT_AT_LEAST via #272, manual) and `ConditionalAlternative.java` provides per-step branching. Remaining gap: true boolean composition (AND/OR/NOT trees) of conditions for a single step — not yet shipped. Scope a follow-up before declaring done.
...
- [/] B4.3 — Per-source backfills (one PR per high-impact candidate from B4.2)  status: in-progress  owner: cha-ndler  updated: 2026-05-13  note: schema extensions shipped — perItemStepDescription (#421) and perItemNpcId (#423). Backfills in flight:
- [x] B4.3.0 — perItemStepDescription schema + coordinator wiring  status: done  owner: cha-ndler  updated: 2026-05-13  pr: #421  note: text-only per-item description override. Includes critical bug fix for B4.1's activeTargetItemId placement.
- [x] B4.3.0b — perItemNpcId schema + coordinator wiring  status: done  owner: cha-ndler  updated: 2026-05-13  pr: #423  note: overlay retarget per-item. 4 getNpcId() coordinator sites updated; static -> instance method for resolveStepIconType.
- [x] B4.3.1 — Shayzien Armour per-item step descriptions  status: done  owner: cha-ndler  updated: 2026-05-13  pr: #422  note: 25 clog items mapped across tiers 1-5 using perItemStepDescription. Tier read directly from existing milestoneKills field — zero wiki guesswork.
- [x] B4.3.2 — Perilous Moons per-Moon retargeting + descriptions  status: done  owner: cha-ndler  updated: 2026-05-14  pr: #425 + #436  note: Uses BOTH perItemStepDescription and perItemNpcId. 12 of 13 clog items map to specific Moon NPC (13011 Blood / 13012 Eclipse / 13013 Blue); atlatl dart (28991) drops from all three so no override. #436 follow-up widened step 1 ARRIVE_AT_TILE distance 20 → 100 and inserted Cam Torum underground intermediate step (Fixes #431) after in-client validation found step 1 didn't auto-fire on quetzal-whistle approach.
- [x] B4.3.3 — Cyclopes Defenders per-item descriptions  status: done         owner: cha-ndler  updated: 2026-05-21  pr: #444  note: 8 defender clog items (Bronze → Dragon, IDs 8844-8850 + 12954) mapped via perItemStepDescription. Lowest-effort Category-1 candidate per #420 ranking.
...
- [x] B4.4 — Top Pick auto-drives per-item method choice (North Star UI alignment)  status: done  owner: cha-ndler  updated: 2026-05-14  pr: #445  note: Top Pick auto-advance carries per-item target ID through `activateGuidance(source, targetItemId)`; closes #420 follow-up. Builds on the perItemRequiredItemIds / perItemStepDescription / perItemNpcId schema shipped in #419/#421/#423.
...
- [x] B.5.5 — Per-step world map arrow + destination icon  status: done       owner: cha-ndler  updated: 2026-05-21  pr: #397 + #438 + #423 + #583  note: All 6 sub-requirements shipped. (1) worldmap arrow per step — #397 (WorldMapDestinationOverlay.setTarget wired in coordinator); (2) chevron shape — #397 (4-point notched polygon); (3) CLH-distinct orange #FF8C00 colour — #583 (CLH_GUIDANCE_ARROW_COLOR constant on WorldMapDestinationOverlay); (4) NPC/object/item icon at destination — #397 (StepIconType enum + 3 sprite variants + resolveStepIconType); (5) worldmap point + arrow integration — #438 (setMapPointActive suppresses double arrow); (6) per-item icon retargeting — #423 (resolveNpcId used in resolveStepIconType).
- [x] B.5.6 — Object/NPC outline-glow highlight style   status: done          owner: cha-ndler  updated: 2026-05-13  pr: #396  note: ModelOutlineRenderer-based glow replaces bounding-box polygons for both NPCs and objects. Closes #377. Validated in-client at the time of merge.
...
- [x] D4 — Long-tail sources deep pass                  status: done          owner: cha-ndler  updated: 2026-05-25  note: All 12 batches merged. Every authored source now sits at the full deep-guidance bar or the relaxed long-tail bar (adopted 2026-05-24). See the 2026-05-25 closure sub-bullet.

  2026-05-24: **Long-tail bar adopted** -- resolves D4 scoping §6 Q1. The relaxed 5-element bar (E1 travel tip, E2 auto-arrival, E6 bank-and-return when applicable, E8 prerequisites, E9 drop-rate confidence) is now formally documented in `docs/contributor-guide/deep-guidance-bar.md` under "Long-tail bar (relaxed, for low-engagement sources)". Approximately 80 of the 144 D4 sources (~55%) will author against the long-tail bar: batches 5, 6, 8, parts of 9 + 10, and the clue rare-reward sub-tables in batch 12. Everything else stays at the full 10-element bar. D4 batches 5+ are unblocked. See this PR for the full doc.

  2026-05-25: **Placeholder / unobtainable source triage complete** -- resolves D4 scoping §6 Q2. Triage pass over all 8 §6 Q2 named candidates (Miscellaneous, My Notes, Port Tasks, Stronghold of Security, Sailing Misc, Boat Paints, Sea Treasures, Ocean Encounters) plus a full scan for any zero-item or single-Placeholder sources. Classification result: **8 (a) real/obtainable**, **0 (b) future placeholder**, **0 (c) removable**. No cleanup PR is needed. Critically, Sailing launched 19 November 2025, so the Q3 watch-item flag on Batch 11 is lifted -- Sailing Misc, Boat Paints, Sea Treasures, and Ocean Encounters are live content. Batch 10 authoring is unblocked for its (a) sources (Miscellaneous, My Notes, Stronghold of Security, Pyramid Plunder, Port Tasks, plus the ~15 other Batch 10 sources from the scoping plan). Full triage rationale and Batch 10/11 scope tables: `docs/contributor-guide/d4-placeholder-triage.md`.

  2026-05-25: **D4 batch 12a merged + next-wave handoff.** Long-tail minigame guidance landed across three PRs — 12a-1 (#673), 12a-2a Fishing Trawler/MTA/Brimhaven/Rogues' Den (#675), 12a-2b Temple Trekking/Vale Totems/Gnome Restaurant x2 (#674). With batches 1-11 already merged (#657/#660/#661/#659/#663/#662/#664/#667/#668/#670/#672/#671), **the only D4 work left is batch 12b + 12c**, after which D4 closes and D5 is next.
  - **Next wave (start a fresh orchestration session here):**
    - **D4 batch 12b — clue rare-reward sub-tables** (~6 sources: Hard / Elite / Master Treasure Trail Rewards (Rare), Shared Treasure Trail Rewards, Scroll Cases, The Mimic). Reduced **long-tail bar** (E1/E2/E6/E8/E9) — guidance lives on the D2 parent clue tiers, so these need the E9 drop-rate-confidence audit but not full E1-E8. Scope table: `docs/contributor-guide/d4-long-tail-scoping.md` §Batch 12.
    - **D4 batch 12c — misc OTHER residue** (~8 sources incl. Random Events, Prifddinas Elf, Fountain of Rune). Mixed bar. Barracuda Trials stays **deferred** (B4.3.5 — re-audit after Sailing mechanics stabilise).
    - On 12b+12c merge: flip D4 to `done`, add the closure note, and open **D5** (retrofit all sources to Tier B/C branch).
  - **Standing constraint:** v1.0.0-hub tag remains **HOLD** (A6) per the user's "fully functional first" decision; the quiet-week countdown restarts on the latest hub-relevant merge (now #682).

  2026-05-25: **Validation / bug-fix session (live `./gradlew run`).** A manual in-game validation pass found and fixed two functional bugs, both merged to master, plus three follow-ups filed. (a) **Login crash** — `PlayerQuestProgressState.onVarbitChanged` called `Quest.getState` (a clientscript) mid-login-script → `AssertionError: scripts are not reentrant`, hanging the client at "Connecting to server..."; fixed by deferring via `ClientThread.invokeLater` with a coalescing guard (#677). (b) **Guidance side-panel flash on every inventory change** — `GuidanceSequencer.onInventoryChanged` re-notified the unchanged step each `ItemContainerChanged`, triggering a two-phase empty→full `StepProgressView` rebuild; fixed by making `StepProgressView.showStep` idempotent and skipping the empty re-notify push, bundled with a flicker-free `removeAll` rebuild and idempotent recommended-chip strip (#682, closes #681). Both validated live (no flash, "Items needed" still updates as items are banked/looted). Follow-ups filed, unfixed by design: **#678** (`drop_rates.json` skill enum `RUNECRAFTING` → `RUNECRAFT`, trivial data fix), **#679** (resolve chip item names on the client thread — the #682 catch only stops the off-EDT crash), **#680** (Requirements row mis-formats "Shades Of Mortton" → "Shades of Mort'ton"). These three are a good low-risk bug-cleanup mini-wave to run alongside or before 12b. See `docs/in-game-validation-log.md` for the validated rows.

  2026-05-25: **D4 closed -- all 12 batches merged.** Batch 12b clue rare-reward sub-tables (#684) + batch 12c misc OTHER residue incl. Random Events (#685) landed, completing the long-tail pass. Coverage check (section-label scan over all 225 sources): every source is authored to the full or long-tail bar except (a) 16 BOSSES sources authored under D2 before the `section`-label convention (full bar, no section fields -- a retrofit candidate for D5 or a small polish pass), (b) Barracuda Trials, deferred (B4.3.5, re-audit post-Sailing), and (c) Shayzien Armour, covered via B4.3.1 perItemStepDescription. Per-source corrections shipped during the close-out: Fountain of Rune spurious Heroes' Quest requirement removed (#685), Runecrafting (Fire Runes) skill enum `RUNECRAFTING` -> `RUNECRAFT` (#687, closes #678), and the validation-session UI fixes #679 (chip-name client-thread resolution) + #680 (Requirements connector-word/apostrophe formatting) via #686. **D5 is now open** -- see its row below.

- [x] D5 — Retrofit all sources to Tier B/C branch      status: done          owner: cha-ndler  updated: 2026-05-26  note: Player-state conditional alternatives layered onto the complete D2-D4 guidance. Merged: D5-A POH slayer/boss (#691), D5-B + overflow POH skilling/wilderness (#692/#693), D5-C diary boss (#690), D5-E teleport jewellery via `inventoryItemIdsAny` (#695), D5-F inventory teleports (#694), D5-H skill-cape perks via new `skillCapePerks` (#697). Schema PR (#696) added `questMilestones` + `skillCapePerks` (both `@Nullable`, fail-closed on unknown enum). D5-D (diary skilling/minigame) and D5-G (quest gates) dropped as null results -- no genuine not-already-wired shortcuts met the >30s bar (D5-G's candidates were either gated at source level already, e.g. Gemstone Rocks/SHILO_VILLAGE, or unlocked no CL-relevant travel). `questMilestones` shipped with zero data users and stays guarded null-everywhere, available for future precision branches. Resolved §5 open questions: charge-aware jewellery = `inventoryItemIdsAny` over charged-tier IDs; per-batch index-agnostic `D5xBranchRegressionTest` classes; both schema fields added.
