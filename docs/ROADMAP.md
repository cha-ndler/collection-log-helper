...
> **Next orchestration session (2026-05-25 handoff).** The active pipeline is **D4 (long-tail deep pass)**; batches 1-11 + 12a are merged. Start the next wave on:
> 1. **D4 batch 12b** — clue rare-reward sub-tables (~6 sources, relaxed long-tail bar). Scope: `docs/contributor-guide/d4-long-tail-scoping.md` §Batch 12.
> 2. **D4 batch 12c** — misc OTHER residue incl. Random Events (~8 sources). Barracuda Trials stays deferred.
> 3. On 12b+12c merge: flip D4 → `done`, then open **D5**.
> 4. **Bug-cleanup mini-wave** (low-risk, can run before/alongside 12b): **#678** (skill-enum data typo, trivial), **#680** (Requirements label formatting), **#679** (resolve chip item names on the client thread — slightly larger).
> Full handoff in the D4 milestone's 2026-05-25 status-log sub-bullet (§9). v1.0.0-hub tag remains HOLD (A6).
...
- [/] D4 — Long-tail sources deep pass                  status: in-progress   owner: cha-ndler  updated: 2026-05-25  note: Long-tail bar adopted 2026-05-24; ~80 of 144 sources author against the relaxed 5-element bar (E1/E2/E6/E8/E9). Batches 1-11 + 12a (10 long-tail minigames) merged. Remaining to close D4: batch 12b (clue rare-reward sub-tables) + batch 12c (misc OTHER residue incl. Random Events). See 2026-05-25 sub-bullet for the next-wave handoff.
