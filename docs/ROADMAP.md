...
- [/] C6 — Wire into top-20 sources                     status: in-progress   owner: cha-ndler  updated: 2026-05-23  note: Scoping doc published in #621 (`docs/contributor-guide/c6-top-20-player-state-wiring-scoping.md`) — 4 batches planned (B0 schema, B1 C2-equipped-items ×13, B2 C1-POH-teleports ×8, B3 C3-diaries ×6 stacking on B1/B2 branches; C4 dropped — zero top-20 applications; C5 covered by existing `quests` field). B0 schema landed in #623: `SourceRequirements` gained nullable `pohTeleports: List<String>` + `equippedItemIds: List<Integer>` fields; `RequirementsChecker.checkRequirementsDetail` evaluates them as AND clauses; 28 new tests (12 Gson + 16 evaluator/alternative-selection). B1/B2/B3 data batches still TODO.
  - 2026-05-23: **B1 batch closed.** C2 equipped-items wiring shipped across the 13 in-scope sources via two data PRs plus a closure doc: PR **#625** wired Ring of Shadows C2 alternatives on DT2 bosses (B1a), PR **#626** wired Drakan's medallion C2 alternatives on ToB / ToB-HM / Nightmare / Phosani (B1b), and PR **#628** recorded the B1 closure and scoped out 5 inventory-teleport sources (deferred to a future C-extension batch outside the original B1 envelope). PR **#627** is a javadoc-scrub follow-up on the B1a regression test. **B2 (C1 POH-teleports, 8 sources)** and **B3 (C3 diaries, 6 sources)** batches in flight.
