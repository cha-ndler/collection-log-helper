...
> **Next orchestration session (2026-05-25 handoff).** **D4 is closed** -- all 12 batches merged; every source sits at the full or long-tail bar. The active pipeline is now **D5 (Tier B/C branch-model retrofit)**. Start the next wave on:
> 1. **Resolve the D5 open questions first** (scoping doc §5): inject-density cap interaction with existing C6 alternatives, charge-aware jewellery fallback (`inventoryItemIdsAny` vs `equippedItemIds`+fallback), regression-test strategy (extend C6 test classes vs new D5 classes), and whether two new schema fields (`skillCapePerks`, `questMilestones`) are strictly needed -- these gate batches D5-A/E/G/H.
> 2. **Dispatch batch D5-A** once (1)-(3) are settled. Scope: `docs/contributor-guide/d5-branch-retrofit-scoping.md` (8 batches D5-A..H, ~85-130 candidate sources).
> 3. D5 layers conditional alternatives / composable conditions onto the now-complete guidance (extends C6's top-20 player-state wiring to the long tail); it does NOT re-author guidance.
> Bug follow-ups still on file from validation: deferred **Barracuda Trials** (B4.3.5, re-audit post-Sailing), and in-client validation of the Sailing coords flagged in #671/#685. v1.0.0-hub tag remains HOLD (A6); quiet-week countdown restarts on the latest hub-relevant merge.
...
- [/] D5 — Retrofit all sources to Tier B/C branch      status: in-progress   owner: cha-ndler  updated: 2026-05-25  note: Opened on D4 close. Scoping doc landed (#688): `docs/contributor-guide/d5-branch-retrofit-scoping.md` -- 8 batches (D5-A..H), ~85-130 candidate sources. D5 LAYERS conditional alternatives / composable conditions onto the now-complete D2-D4 guidance (extends C6's top-20 player-state wiring to the long tail); it does NOT re-author guidance. Resolve four §5 open questions before batch D5-A: inject-density cap vs existing C6 alternatives, charge-aware jewellery fallback (`inventoryItemIdsAny` vs `equippedItemIds`+fallback), regression-test strategy, and whether two new schema fields (`skillCapePerks`, `questMilestones`) are strictly needed.
