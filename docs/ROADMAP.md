...
- [?] A6 — Tag v1.0.0-hub, wait a week, resubmit         status: blocked       owner: cha-ndler  updated: 2026-05-19  pr: #355, refresh pending  note: **HOLD — do not tag or resubmit.** 2026-05-16 validation issues: 5/6 closed — #483 (clue panel overlap), #484 (AKQ→AIQ data fix), #485 (P0 ARRIVE_AT_TILE auto-advance regression), #486 (C7 debug overlay completeness), #488 (F1/F2 sync UX). **#487 (C5 quest count mismatch) remains OPEN** (P2, label-only fix per maintainer call). 2026-05-19 hot-fix: PR #543 added `@ImplementedBy` to four C-tier player-state interfaces (`DiaryTierState`, `EquippedItemState`, `PohTeleportInventory`, `SkillCapePerkState`) after live `./gradlew run` surfaced a `PluginInstantiationException` regression latent since PRs #459/#461 — DI was unbound for those four interfaces and any Plugin-Hub build would have failed to load on a fresh client. Regression guard `CollectionLogHelperPluginInjectorTest` added to catch this class of bug at CI time. The 2026-05-19 earliest-push target remains voided; quiet-week countdown restarts only after a clean re-run of validation log Phases 2-5. User decision 2026-05-16: "i don't want to deploy this to the plugin hub until things are fully functional."
...
- [/] B4.3.3 — Cyclopes Defenders per-item descriptions  status: in-progress  owner: cha-ndler  updated: 2026-05-14  note: data PR in flight. 8 defender clog items mapped (Bronze → Dragon chain) via perItemStepDescription. Lowest-effort Category-1 candidate per #420 ranking.
